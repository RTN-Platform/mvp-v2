...
// Define the form schema for accommodations
export const accommodationFormSchema = z.object({
  ...
  price_per_person: z.number().positive({ message: "Price must be greater than 0" }).optional(),
...
// Define experience form schema to support both types of listings
export const experienceFormSchema = z.object({
  title: z.string().min(5, { message: "Title must be at least 5 characters long" }),
  description: z.string().min(20, { message: "Description must be at least 20 characters long" }),
  location: z.string().min(3, { message: "Location is required" }),
  price_per_person: z.number().positive({ message: "Price must be greater than 0" }),
  duration: z.number().int().min(1, { message: "Duration must be at least 1 hour" }),
  capacity: z.number().int().min(1, { message: "Must accommodate at least 1 guest" }),
